...
class Tracker:
    ...
    def interoplate_ball_position(self, ball_positions):
    ...
    def get_object_tracks(self, frames, read_from_stub, stub_path=None):

        if read_from_stub and stub_path is not None and os.path.exists(stub_path):
            with open(stub_path, 'rb') as f:
                tracks = pickle.load(f)
            print(f"FOUND EXISTING TRACKS IN {stub_path}")
            return tracks
        ...
        if stub_path is not None:
            ...
            print("SAVED PICKLE FILE OF TRACKS")
        return tracks
        print(detection_supervision)

    def draw_ellipse(self, frame, bbox, color, id):
        y2 = int(bbox[3])
